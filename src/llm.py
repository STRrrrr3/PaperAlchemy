...
# 强制忽略 SSL 证书校验，避免本机代理/证书环境导致的握手失败打断调试。
try:
...
def _load_vertex_runtime() -> dict[str, Any] | None:
    ...
    try:
        ...
        raise ValueError(f"检测到 Vertex service account JSON，但加载失败：{exc}") from exc
    ...
    if not project_id:
        raise ValueError("Vertex service account JSON 中缺少 project_id，且环境变量也未提供。")
...
def get_llm(temperature: float = 0, use_smart_model: bool = True):
    ...
    if not os.getenv("GOOGLE_API_KEY"):
        raise ValueError(
            "未找到可用的 Gemini 凭据：既没有 Vertex service account JSON，也没有 GOOGLE_API_KEY。"
        )
